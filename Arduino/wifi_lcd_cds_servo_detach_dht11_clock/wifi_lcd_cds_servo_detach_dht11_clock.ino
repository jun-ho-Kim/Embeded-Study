...
#define SERVER_NAME "10.10.14.52"
...
void loop() {
  ...
  if (timerIsrFlag) //1초에 한번씩 실행
  {
    ...
    sprintf(lcdLine1, "%02d.%02d  %02d:%02d:%02d", 8, 30, 19, dateTime.min, dateTime.sec );
    lcdDisplay(0, 0, lcdLine1);
    if (updatTimeFlag)
    {
...
void socketEvent()
{
  ...
  Serial.print("pArray[1] is ");
  Serial.println(pArray[1]);

  Serial.print("pArray[2] is ");
  Serial.println(pArray[2]);
  int level = atoi(pArray[1]);
  Serial.println(level);
  //[KSH_ARD]LED@ON : pArray[0] = "KSH_ARD", pArray[1] = "LED", pArray[2] = "ON"
  Serial.print("pArray[0] is");
  Serial.println(pArray[0]);
  Serial.print("atoi(pArray[20]) < 30.0");
  Serial.println(atoi(pArray[20]) < 30.0);
    Serial.print("pArray[2] is");
  Serial.println(pArray[2]);
  ...
  if ((strlen(pArray[1]) + strlen(pArray[2])) < 16)
  {
    ...
    if(atoi(pArray[2]) < 30.0 )
    {
      sprintf(lcdLine2, "%scm ClOSE", pArray[2]);
      if(isLedState == 1)
        digitalWrite(LED_LAMP_PIN, HIGH);
      isLedState = 0;
    }
    else if(atoi(pArray[2]) >= 30.0 )
    {
      sprintf(lcdLine2, "%scm OPEN", pArray[2]);
      if(isLedState == 0)
