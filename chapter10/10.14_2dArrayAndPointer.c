...
int main()
{
	...
	//printf("%u\n", (unsigned)arr2d); // Use unsign long long in x64
	//printf("%u\n", (unsigned)arr2d[0]);
	//printf("\n");
	...
	float(*pa)[4]; // a SINGLE pointer to an array of 2 float
	float* ap[2]; // an array of TWO pointers-to-float

	printf("%zu\n", sizeof(pa)); // 4 // 8 in x64
	printf("%zu\n", sizeof(ap)); // 8 // 16 in x64
	printf("\n");
	...
	printf("%u %u\n", (unsigned)ap, (unsigned)(ap + 1)); // 포인터 배열이므로 주소값이 다르다.
