...
module_param(timevalue,int,0);
struct timer_list timerLed;

void kerneltimer_timeover(unsigned long timeover);
...
//    kerneltimer_registertimer(timerVal);
    mod_timer(t, get_jiffies_64() + timevalue);
}
    ...
    switch(cmd)
    {
        case TIMER_START :
            if(!timer_pending(&timerLed))
                kerneltimer_registertimer(timevalue);
	        //ret=copy_from_user(uf,(void *)buf,count);
            break;
        case TIMER_STOP : 
            if(timer_pending(&timerLed))
                del_timer(&timerLed);
            break;
        case TIMER_VALUE:
          copy_from_user((void *)&keydata_info,(void *)arg,(unsigned long)sizeof(keydata_info));
          timevalue = (unsigned long)keydata_info.timer_val;
          kerneltimer_registertimer(timevalue);
        default:
            err =-E2BIG;
            break;
